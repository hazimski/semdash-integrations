...
-- Create shared_domain_history table if not exists
...
-- Add indexes
create index if not exists shared_domain_history_token_idx on public.shared_domain_history(share_token);
create index if not exists shared_domain_history_expires_idx on public.shared_domain_history(expires_at);
...
create or replace function public.get_shared_domain_history(p_share_token uuid)
returns table (
  domain text,
  location_code text,
  language_code text,
