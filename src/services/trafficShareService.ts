...
interface TrafficShareResponse {
  tasks: Array<{
    result: Array<{
      total_count: number;
      items: Array<{
...
export async function fetchTrafficShare(
  ...
  try {
    ...
    const response = await api.post<TrafficShareResponse>(
    ...
    console.error('Traffic Share API Error:', error);
