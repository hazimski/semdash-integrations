...
import { PageSummaryDropdown } from '../../components/backlinks/PageSummaryDropdown';
...
export function ResultsTable({ 
  ...
  const handleBacklinksClick = (page: string) => {
    // Clean the URL to get domain and path
    const url = new URL(page.startsWith('http') ? page : `https://${page}`);
    ...
    // Only add filter if path exists
    if (path) {
      params.initialFilter = JSON.stringify({
        type: 'all',
        urlTo: path
      });
    }
            ...
            {pages.map((page, index) => (
              <tr key={index} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  <button
                    onClick={() => handleBacklinksClick(page.page)}
