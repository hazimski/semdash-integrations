...
interface SingleTrafficShareTableProps {
  data: Array<{
    domain: string;
    etv: number;
...
export function SingleTrafficShareTable({ 
  data = [], 
  totalCount = 0,
  currentPage,
  onPageChange,
  ...
  error,
  itemsPerPage = 100
}: SingleTrafficShareTableProps) {
  const totalPages = Math.ceil(totalCount / itemsPerPage);

  // Calculate total ETV for percentage calculations
  const totalEtv = data.reduce((sum, item) => sum + item.etv, 0);
  ...
  const handleExport = () => {
    ...
    const csvData = data.flatMap(item => 
      Object.entries(item.keywords_positions).map(([keyword, positions]) => [
        item.domain,
        item.etv,
        `${((item.etv / totalEtv) * 100).toFixed(1)}%`,
        item.keywords_count,
        keyword,
        positions.join(' & ')
      ])
    ...
    saveAs(blob, 'single_traffic_share_analysis.csv');
  };
  ...
  const expandedData = data.flatMap(item => 
    Object.entries(item.keywords_positions).map(([keyword, positions]) => ({
      ...
      keyword,
      positions,
      trafficShare: (item.etv / totalEtv) * 100
    }))
  );
