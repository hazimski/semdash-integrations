...
export function CompetitorDomains() {
  ...
  const currentParams = {
    ...
    location: searchParams.get('location') || '2840', // Default to US
    language: searchParams.get('language') || 'en'    // Default to English
  };
  ...
  const handleAnalyze = async (domain: string, location: string, language: string, page: number = 1) => {
    ...
    try {
      ...
      const data = await fetchCompetitors(domain, location, language, itemsPerPage, offset);
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">Competitor Domains</h1>
