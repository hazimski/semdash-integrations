...
import { BulkTrafficShareFilters } from './BulkTrafficShareFilters';
import { BulkTrafficShareTable } from './BulkTrafficShareTable';
...
export function BulkTrafficShareResults() {
  ...
  return (
    <div className="space-y-6">
      <nav className="flex items-center text-sm text-gray-500">
        <Link to="/bulk-traffic-share" className="hover:text-gray-700">
          Competition Analysis
        </Link>
        <ChevronRight className="w-4 h-4 mx-2" />
        <span>Bulk Traffic Share</span>
      </nav>

      <div className="space-y-4">
        <h1 className="text-2xl font-bold text-gray-900">
          Domain Traffic Share for: {currentParams.keywords.join(', ')}
        </h1>
      ...
      {error ? (
        <ErrorState 
          title="No traffic share data found"
          message="We couldn't find any traffic share data. This could be because:"
          suggestions={[
            'The keywords have very low search volume',
            'The filters applied are too restrictive',
            'There are no domains ranking for these keywords'
          ]}
        />
      ) : (
        <BulkTrafficShareTable 
          data={data}
