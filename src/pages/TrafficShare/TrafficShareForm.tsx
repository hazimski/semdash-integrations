...
interface TrafficShareFormProps {
  onSearch: (keywords: string[], location: string, language: string, includeSubdomains: boolean) => void;
}

export function TrafficShareForm({ onSearch }: TrafficShareFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // If not in bulk mode, only take the first keyword
    const finalKeywords = isBulkMode ? keywordList : [keywordList[0]];

    setIsLoading(true);
    onSearch(finalKeywords, location, language, includeSubdomains);
    setIsLoading(false);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold text-gray-900">Traffic Share Analysis</h2>
        <div className="flex items-center space-x-3">
          <span className={`text-sm ${!isBulkMode ? 'text-blue-600 font-medium' : 'text-gray-500'}`}>Single</span>
          <Switch
            checked={isBulkMode}
          ...
          <span className={`text-sm ${isBulkMode ? 'text-blue-600 font-medium' : 'text-gray-500'}`}>Bulk</span>
        </div>
      </div>

      <div>
        <label htmlFor="keywords" className="block text-sm font-medium text-gray-700 mb-1">
          {isBulkMode ? 'Keywords (one per line)' : 'Keyword'}
        </label>
        {isBulkMode ? (
          <textarea
            id="keywords"
